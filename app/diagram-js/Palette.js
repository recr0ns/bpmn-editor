...
const info = domify(`<div class="djs-pallete-info">
<h2>Items</h2>
<p>Tap or drag items into workspace</p></div>`);

const allowed_groups =['services', 'skills', 'other'];
Palette.prototype._update = function() {
  ...
  entriesContainer.appendChild(info);
  console.log(entries);
  forEach(entries, function(entry, id) {

    var grouping = entry.group || 'default';import {
      isFunction,
      isArray,
      forEach
    } from 'min-dash';
    ...
    Palette.prototype._update = function() {
      ...
      forEach(entries, function(entry, id) {
        ...
        if (!allowed_groups.includes(grouping)) return;
    
        var container = domQuery('[data-group=' + grouping + ']', entriesContainer);
        if (!container) {
          container = domify('<div class="group" data-group="' + grouping + '"><h3>'+ (entry.groupName || grouping)+'</h3></div>');
        ...
        if (!entry.separator) {
          ...
          if (entry.element) {
            control.appendChild(domify(entry.element));
          }
        }
      });
